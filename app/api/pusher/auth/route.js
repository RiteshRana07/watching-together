...
const { getRoomByCode, reserveRoomSeat, getUserById } = require("../../../../lib/db");
...
export async function POST(req) {
  ...
  const reservation = await reserveRoomSeat(code, payload.userId, dbUser?.email);
  if (!reservation.ok) {
  ...
  return NextResponse.json(
    pusher.authorizeChannel(socketId, channelName, {
      ...
      user_info: { username: payload.username, isHost, isSuperHost },
    })
  );
}
