...
function apiHost() {
  return process.env.PCLOUD_API_HOST || "api.pcloud.com";
}
...
async function pcloudGet(method, params = {}) {
  ...
  const res = await fetch(`https://${apiHost()}/${method}?${qs.toString()}`);
...
async function uploadFileProxy(file, filename) {
  ...
  const res = await fetch(
    `https://${apiHost()}/uploadfile?access_token=${token}&folderid=${folderId}`,
