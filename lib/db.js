...
const { isSuperHostEmail } = require("./superhost");
...
function ensureSchema() {
  if (!schemaReady) {
    schemaReady = pool.query(`
      ...
      -- Co-host status persisted (not just broadcast) so the server can
      -- actually authorize co-host actions, and so it survives a refresh.
      ...
      -- Persisted chat, but only ever READ BACK for the host (or the
      -- site-wide super-host) — everyone else only sees messages sent
      -- live during their own visit. Deleted along with the room.
...
async function deleteRoom(code, userId, email) {
  await ensureSchema();
  if (isSuperHostEmail(email)) {
    await pool.query(`DELETE FROM rooms WHERE code = $1`, [code]);
    return;
  }
  await pool.query(`DELETE FROM rooms WHERE code = $1 AND host_id = $2`, [code, userId]);
}

async function updateRoomCapacity(code, userId, maxParticipants, email) {
  await ensureSchema();
  if (isSuperHostEmail(email)) {
    const { rows } = await pool.query(
      `UPDATE rooms SET max_participants = $1 WHERE code = $2 RETURNING *`,
      [maxParticipants, code]
    );
    return rows[0] || null;
  }
  const { rows } = await pool.query(
    `UPDATE rooms SET max_participants = $1 WHERE code = $2 AND host_id = $3 RETURNING *`,
    [maxParticipants, code, userId]
  );
  return rows[0] || null;
}

async function reserveRoomSeat(code, userId, email) {
  await ensureSchema();
  const client = await pool.connect();
  const now = Date.now();
  ...
  try {
    ...
    const isHost = room.host_id === userId || isSuperHostEmail(email);
...
async function playNextRoomQueueItem(code, userId, email) {
  await ensureSchema();
  const client = await pool.connect();
  try {
    await client.query("BEGIN");
    ...
    // Host, co-host, or the site-wide super-host may advance the queue.
    let allowed = room.host_id === userId || isSuperHostEmail(email);
    if (!allowed) {
      const coHostRes = await client.query(
        `SELECT 1 FROM room_cohosts WHERE room_id = $1 AND user_id = $2`,
        [room.id, userId]
      );
      allowed = coHostRes.rows.length > 0;
    }
    if (!allowed) {
      await client.query("ROLLBACK");
      return { error: "Only the host or a co-host can play the next video" };
    }
...
// The one check used everywhere someone should be allowed to act with
// host-level power in a room: the room's actual host, a co-host the host
// promoted, OR the site-wide super-host account (see lib/superhost.js) —
// which gets these powers in EVERY room without ever being recorded as
// that room's host_id, so the real creator keeps their own ownership.
async function canManageRoom(room, userId, email) {
  if (room.host_id === userId) return true;
  if (isSuperHostEmail(email)) return true;
  await ensureSchema();
  const { rows } = await pool.query(
    `SELECT 1 FROM room_cohosts WHERE room_id = $1 AND user_id = $2`,
    [room.id, userId]
